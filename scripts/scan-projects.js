...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const outputPath = path.join(__dirname, '..', 'src', 'data', 'projects-data.json')
  
  fs.mkdirSync(path.dirname(outputPath), { recursive: true })
  fs.writeFileSync(outputPath, JSON.stringify(result, null, 2))
  
  console.log('✅ Projects scanned and saved to', outputPath)
  
  function countItems(items) {
    let count = { folders: 0, projects: 0 }
    for (const item of items) {
      if (item.type === 'folder') {
        count.folders++
        const childCount = countItems(item.children)
        count.folders += childCount.folders
        count.projects += childCount.projects
      } else {
        count.projects++
      }
    }
  ...
  const companyCount = countItems(result.company)
  const portfolioCount = countItems(result.portfolio)
  
  console.log(`  - Company: ${companyCount.folders} folders, ${companyCount.projects} projects`)
  console.log(`  - Portfolio: ${portfolioCount.folders} folders, ${portfolioCount.projects} projects`)
}

export { scanProjects, isSpecialFolder, formatFolderName }
